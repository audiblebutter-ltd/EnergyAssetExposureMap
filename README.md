...
Maps Britain's energy infrastructure (offshore wind, oil & gas platforms, onshore solar/wind/battery/bioenergy/hydro, nuclear, power infrastructure) against natural hazard data (storm tracks, flood zones, seismic activity, wildfire history) to answer a simple question: what assets are on risk, and where. Built entirely from public data sources, with the pipeline and output left open for anyone to explore.
...
**Join**
- A join Lambda, triggered by S3 `ObjectCreated` on `raw/`, reads the latest asset and hazard snapshots and joins them by proximity (haversine distance to the nearest hazard, polygons reduced to a centroid - see `shared/geo.py` for why this is v1 and not true polygon intersection). Wildfire perimeters are joined the same way as flood/earthquake (real per-event geometry, nearest distance). Storms are the one national-scale hazard (no per-storm geometry), so they're summarized per asset rather than distance-joined.
...
Data engineering is built, **deployed, and verified end-to-end in AWS**: stack `energy-asset-exposure-map-dev` in `eu-west-2`. `processed/latest.json` currently holds 3,450 real energy assets - 72 Crown Estate England/Wales/NI leases, 58 Crown Estate Scotland leases, 265 NSTA platforms, 3,050 operational onshore sites from DESNZ REPD (1,391 solar, 778 onshore wind, 270 landfill gas, 171 battery, 151 anaerobic digestion, and more), and 5 nuclear stations - each joined against flood/earthquake/wildfire/storm hazard data. Adding onshore assets took flood exposure from a structural 0% (every prior asset was offshore/coastal) to a real 1.0% (33/3,450), and wildfire - a hazard the pipeline didn't cover at all before - now shows a real 8.3% (286/3,450) exposed within 5km of a 2016-2025 burnt-area perimeter. The processed bucket is public-read (scoped to `processed/*`), and the interactive map frontend and write-up are live at [dataandgrit.energy/energy-asset-exposure-map](https://dataandgrit.energy/energy-asset-exposure-map).
...
- EFFIS wildfire coverage only detects burns picked up by MODIS/VIIRS satellite thermal-anomaly detection - very small fires can go undetected, and `EFFIS_YEARS` needs a manual bump each January for the new year's layer (same manual-maintenance shape as the REPD URL).
